...
bbdd_covar %>%
  ...
  bind_rows(bbdd_covar %>%
              ...
              filter(value != 0)) %>% 
  bind_rows(bbdd_covar %>%
              select(-sex_female, -Current, -obesity_BMI, -Former, -obesity, -A10, -A10A, -A10B,
                     -C02, -C03, -C07, -C08, -C09, -C10, -M01A, -angor, -ami, -stroke, -tia, -T2DM) %>% 
              pivot_longer(cols = !rowId) %>%
              group_by(name) %>%
...
ggplot(bbdd_covar %>%
         select(rowId, age, BMI, HbA1c, Leukocytes, Monocytes, cLDL, Tg, cHDL, DBP, SBP, Glucose,
                Ferritin, TimeT2DM, sex_female, obesity_BMI) %>%
...
ggplot(bbdd %>%
         select(age, BMI, HbA1c, Leukocytes, Monocytes, cLDL, Tg, cHDL, DBP, SBP, Glucose,
                TimeT2DM, Ferritin) %>%
         cor %>%
         reshape2::melt(),
...
map_dfr(2:15, function(nclus){
  data.frame(cluster = fxclus(bbdd_resid %>% select(-rowId),
                              nclus)$cluster) %>%
    count(cluster) %>%
    mutate(n = 100 * (n/sum(n))) %>%
    slice_min(n, with_ties = FALSE) %>%
