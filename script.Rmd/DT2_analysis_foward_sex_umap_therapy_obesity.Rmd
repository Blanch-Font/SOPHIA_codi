...
```{r}
bbdd_covar_therapy <- list(bbdd_covar %>% filter(A10 ==1), bbdd_covar %>% filter (A10A == 1), bbdd_covar %>% filter (A10B == 1),
...
names(bbdd_covar_therapy) <- c("A10", "A10A", "A10B", "C02", "C03", "C07", "C08", "C09", "C10", "M01A")
...
bbdd_covar_therapy <- rbind(bbdd_covar_therapy$A10,bbdd_covar_therapy$A10A, bbdd_covar_therapy$A10B, bbdd_covar_therapy$C02,
                            bbdd_covar_therapy$C02, bbdd_covar_therapy$C03, bbdd_covar_therapy$C07, bbdd_covar_therapy$C08, bbdd_covar_therapy$C09,
                            bbdd_covar_therapy$C10, bbdd_covar_therapy$M01A)
...
clusters_men$THERAPY <- relevel(clusters_men$THERAPY, ref= "C10") #REFERENCIA MENOR SUPERVIVENCIA

clusters_women <- bbdd_covar_therapy[[2]]
clusters_women$THERAPY <- relevel(clusters_women$THERAPY, ref = "C10")
...
```{r}
herapies<- levels(bbdd_covar_therapy[[1]]$THERAPY)
bbdd_covar_therapy %>% map(function(ther){
  for (x in 1:length(therapies)){
    for (y in 1:length(therapies)){
      if (y <= x){
