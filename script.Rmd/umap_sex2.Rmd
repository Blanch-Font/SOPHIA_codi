...
bbdd %>%
  select(age, BMI, HbA1c, Leukocytes, Monocytes, cLDL, Tg, cHDL, DBP, SBP, Glucose, sex_female) %>%
  group_by(sex_female) %>% 
  group_modify(~{.x %>%
      cor %>%
      reshape2::melt()
...
umap_resid_50 <- bbdd_resid %>%
  select(HbA1c, Leukocytes, Monocytes, cLDL, Tg, cHDL, DBP, SBP, Glucose, sex_female) %>%
  group_by(sex_female, .add = T) %>%
  group_split() %>% 
  map(function(strat, ...){
    umap(X = strat,
