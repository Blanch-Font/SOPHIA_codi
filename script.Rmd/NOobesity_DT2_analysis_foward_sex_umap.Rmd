---
title: "T2DM_analysis no obesity"
...
bbdd_covar <- bbdd_covar %>% filter(obesity_BMI == "No obesity")
```
...
clusters_women$name <- relevel(clusters_women$name, ref = "Cluster 6")
...
clusters_men <- diseases[[1]] 
clusters_men <- clusters_men %>% filter(name != "Cluster 10", name != "Cluster 9", name != "Cluster 7")
clusters_men$name <- as.factor(clusters_men$name)
...
clusters_women$name <- as.factor(clusters_women$name)
clusters_women <- clusters_women %>% filter(name != "Cluster 7", name != "Cluster 4", name != "Cluster 1", name != "Cluster 2")
clusters_women$name <- relevel(clusters_women$name, ref = "Cluster 5")
...
clusters_men <- diseases[[1]] 
clusters_men <- clusters_men %>%filter( name != "Cluster 9", name != "Cluster 2", name != "Cluster 7")
clusters_men$name <- as.factor(clusters_men$name)
clusters_men$name <- relevel(clusters_men$name, ref="Cluster 1") #REFERENCIA MAYOR SUPERVIVENCIA

clusters_women <- diseases[[2]]
clusters_women$name <- as.factor(clusters_women$name)
clusters_women <- clusters_women %>%filter( name != "Cluster 9")
...
clusters_men <- diseases[[1]] 
clusters_men <- clusters_men %>% filter (name != "Cluster 7",name != "Cluster 6", name != "Cluster 3", name != "Cluster 11", 
                                         name != "Cluster 10", name != "Cluster 8")
clusters_men$name <- as.factor(clusters_men$name)
clusters_men$name <- relevel(clusters_men$name, ref="Cluster 4") #REFERENCIA MAYOR SUPERVIVENCIA

clusters_women <- diseases[[2]]
clusters_women$name <- as.factor(clusters_women$name)
clusters_women <- clusters_women %>% filter(name != "Cluster 1",  name != "Cluster 7",  name != "Cluster 9")
...
```{r}
bbdd_clusters_5 <- create_bbdd_clusters(gmm_5)
head(bbdd_clusters_5)
```

### STROKE
```{r}
bbdd_clusters_5 %>% group_by(sex_female) %>% group_split %>% map(function(data){
...
```{r}
summary(as.factor(bbdd_clusters_5$clusters))

bbdd_clusters_5$clusters <- as.factor(bbdd_clusters_5$clusters)

clusters_men <- bbdd_clusters_5 %>% filter(sex_female=="Men") #FILTRO CUANDO NO APARECE EVENTO
clusters_men$clusters <- relevel(clusters_men$clusters, 1) #REFERENCIA MAYOR SUPERVIVENCIA

clusters_women <- bbdd_clusters_5 %>% filter(sex_female=="Women")
clusters_women$clusters <- relevel(clusters_women$clusters, ref = 1)

clusters = list(clusters_men, clusters_women)

bbdd_clusters_5_surv <- clusters %>%
  map(function(data) {mod = coxph(Surv(t.ep_StrokeI, i.ep_StrokeI)~age + BMI+ clusters, data=data)
      tidy(mod, conf.int= TRUE, exponentiate=TRUE)})
head(bbdd_clusters_5_surv)
```

```{r}
options(repr.plot.width = 9, repr.plot.height = 3)
bbdd_clusters_5_surv[[1]] %>%
    ggplot(aes(estimate, term)) +
    geom_vline(xintercept = 1, lty = "dashed") +
    geom_linerange(aes(xmin = conf.low, xmax = conf.high)) +
    geom_point(aes(color = term)) +
...
clusters_men <- bbdd_clusters_5 %>% filter(sex_female=="Men") #FILTRO CUANDO NO APARECE EVENTO
clusters_men$clusters <- relevel(clusters_men$clusters, ref = 1) #REFERENCIA MAYOR SUPERVIVENCIA
...
clusters_women <- bbdd_clusters_5 %>% filter(clusters != 2, clusters != 1,sex_female=="Women")
clusters_women$clusters <- relevel(clusters_women$clusters, ref = 1)
...
clusters_men <- bbdd_clusters_5 %>% filter(sex_female=="Men") #FILTRO CUANDO NO APARECE EVENTO
clusters_men$clusters <- relevel(clusters_men$clusters,ref = 4) #REFERENCIA MAYOR SUPERVIVENCIA
...
### 6 CLUSTERS
```{r}
bbdd_clusters_5 %>% group_by(sex_female, .add=TRUE) %>% group_split() %>% map(function(dat){
...
bbdd_clusters_5 %>% group_by(sex_female, .add=TRUE) %>% group_split() %>% map2(bbdd_covar %>% group_by(sex_female) %>% group_split,
                                                                               function(dat, weight){
