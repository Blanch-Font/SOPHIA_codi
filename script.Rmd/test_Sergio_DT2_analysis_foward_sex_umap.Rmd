...
```{r}
diseases %>% map(function(clus){
  table_prob <- table(clus$i.ep_AMI,clus$name)
  print(table_prob)
fisher.test(table_prob, simulate.p.value = TRUE)
})
```
...
diseases %>% map(function(clus){
  table_prob <- table(clus$i.ep_AMI,clus$name)
  table_prob <- as.data.frame(table_prob)
  require(coin)
  symmetry_test(Freq ~ factor(Var2) | factor(Var1), data=table_prob, teststat = "quad")
})
```

```{r}
diseases %>% map(function(clus){
  table_prob <- table(clus$i.ep_TIA,clus$name)
  print(table_prob)
fisher.test(table_prob, simulate.p.value = TRUE)
})
```
...
```{r}
diseases %>% map(function(clus){
  table_prob <- table(clus$i.ep_StrokeI,clus$name)
  print(table_prob)
fisher.test(table_prob, simulate.p.value = TRUE)
})
```
```{r}
diseases %>% map(function(clus){
  table_prob <- table(clus$i.ep_Angor,clus$name)
  print(table_prob)
fisher.test(table_prob, simulate.p.value = TRUE)
})
```
...
```{r}
bbdd_clusters_8_sup <- create_bbdd_clusters(gmm_8)

bbdd_clusters_8_sup <- bbdd_clusters_8_sup %>%
  left_join(bbdd_covar %>% select(rowId, CKD, COPD, cancer, depress, htn, hf, liver, ra, sleep_apnea, pcos),  by = "rowId")


bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(arche){
  ...
  table_CKD <- as.data.frame(prop.table(table(arche$clusters, arche$CKD), margin=1))
  table_CKD <- table_CKD %>% filter(Var2 == 1) 
  colnames(table_CKD) <- c("Cluster", "Disease", "Freq")
  table_CKD <- table_CKD %>% mutate(Disease=recode(Disease, '1'="CKD"))
  
  table_COPD <- as.data.frame(prop.table(table(arche$clusters, arche$COPD), margin=1))
  table_COPD <- table_COPD %>% filter(Var2 == 1) 
  colnames(table_COPD) <- c("Cluster", "Disease", "Freq")
  table_COPD <- table_COPD %>% mutate(Disease=recode(Disease, '1'="COPD"))
  
  table_cancer <- as.data.frame(prop.table(table(arche$clusters, arche$cancer), margin=1))
  table_cancer <- table_cancer %>% filter(Var2 == 1) 
  colnames(table_cancer) <- c("Cluster", "Disease", "Freq")
  table_cancer <- table_cancer %>% mutate(Disease=recode(Disease, '1'="cancer"))
  
  table_htn <- as.data.frame(prop.table(table(arche$clusters, arche$htn), margin=1))
  table_htn <- table_htn %>% filter(Var2 == 1) 
  colnames(table_htn) <- c("Cluster", "Disease", "Freq")
  table_htn <- table_htn %>% mutate(Disease=recode(Disease, '1'="htn"))
  
  table_hf <- as.data.frame(prop.table(table(arche$clusters, arche$hf), margin=1))
  table_hf <- table_hf %>% filter(Var2 == 1) 
  colnames(table_hf) <- c("Cluster", "Disease", "Freq")
  table_hf <- table_hf %>% mutate(Disease=recode(Disease, '1'="hf"))
  
  table_liver <- as.data.frame(prop.table(table(arche$clusters, arche$liver), margin=1))
  table_liver <- table_liver %>% filter(Var2 == 1) 
  colnames(table_liver) <- c("Cluster", "Disease", "Freq")
  table_liver <- table_liver %>% mutate(Disease=recode(Disease, '1'="liver"))
  
    table_ra <- as.data.frame(prop.table(table(arche$clusters, arche$ra), margin=1))
  table_ra <- table_ra %>% filter(Var2 == 1) 
  colnames(table_ra) <- c("Cluster", "Disease", "Freq")
  table_ra <- table_ra %>% mutate(Disease=recode(Disease, '1'="ra"))
  
      table_pcos <- as.data.frame(prop.table(table(arche$clusters, arche$pcos), margin=1))
  table_pcos <- table_pcos %>% filter(Var2 == 1) 
  colnames(table_pcos) <- c("Cluster", "Disease", "Freq")
  table_pcos <- table_pcos %>% mutate(Disease=recode(Disease, '1'="pcos"))
  
      table_sleep_apnea <- as.data.frame(prop.table(table(arche$clusters, arche$sleep_apnea), margin=1))
  table_sleep_apnea <- table_sleep_apnea %>% filter(Var2 == 1) 
  colnames(table_sleep_apnea) <- c("Cluster", "Disease", "Freq")
  table_sleep_apnea <- table_sleep_apnea %>% mutate(Disease=recode(Disease, '1'="sleep_apnea"))
  
  table <- rbind(table_AMI, table_TIA, table_StrokeI, table_Angor, table_COPD, table_CKD, table_cancer, table_htn, 
                 table_hf, table_liver, table_ra, table_sleep_apnea, table_pcos)
  print(table)
  ggplot(table, aes(x=factor(Cluster), y=Freq, fill=factor(Disease))) + geom_bar(stat="identity", position= "dodge") +
    ggtitle("Probability inside cluster") + xlab("Cluster") + ylab("Probability") + labs(fill="Disease")
})
...
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- prop.table(table(clus$CKD,clus$clusters))
  barplot(table_prob, beside=TRUE, legend.text = rownames(table_prob), main=paste("CKD", if_else(clus$sex_female[1] == "Men","MALE", "FEMALE")),
          col=c("green","red"))
})
```
...
```{r, fig.width=10, fig.height=4}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- prop.table(table(clus$CKD,clus$clusters), margin=2)
  table_prob <- as.data.frame(table_prob)
  table_prob <- table_prob %>% filter (Var1==1) %>% select(Var2, Freq)
  prob <- table_prob$Freq
  names(prob) <- table_prob$Var2
  barplot(prob,legend.text = rownames(prob), main=paste(c("CKD"), if_else(clus$sex_female[1] == "Men", "MALE","FEMALE" )),
          col=c("purple"))
})
```
...
```{r, fig.width=10, fig.height=4}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- prop.table(table(clus$COPD,clus$clusters))
  barplot(table_prob, beside=TRUE, legend.text = rownames(table_prob), main=paste("COPD", if_else(clus$sex_female[1] == "Men","MALE", "FEMALE")),
          col=c("green","red"))
})
```
...
```{r, fig.width=10, fig.height=4}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- prop.table(table(clus$COPD,clus$clusters), margin=2)
  table_prob <- as.data.frame(table_prob)
  table_prob <- table_prob %>% filter (Var1==1) %>% select(Var2, Freq)
  prob <- table_prob$Freq
  names(prob) <- table_prob$Var2
  barplot(prob,legend.text = rownames(prob), main=paste(c("COPD"), if_else(clus$sex_female[1] == "Men", "MALE","FEMALE" )),
          col=c("purple"))
})
```
...
```{r, fig.width=10, fig.height=4}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- prop.table(table(clus$cancer,clus$clusters))
  if(nrow(table_prob)>1){
  barplot(table_prob, beside=TRUE, legend.text = rownames(table_prob), main=paste("CANCER", if_else(clus$sex_female[1] == "Men","MALE", "FEMALE")),
          col=c("green","red"))
  }
  else{
      barplot(table_prob, beside=TRUE, legend.text = rownames(table_prob), main=paste("CANCER", if_else(clus$sex_female[1] =="Men",
                                                                                                        "MALE","FEMALE")),
          col=c("green"))
  }
})
...
```{r, fig.width=10, fig.height=4}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- prop.table(table(clus$cancer,clus$clusters), margin=2)
  table_prob <- as.data.frame(table_prob)
  table_prob <- table_prob %>% filter (Var1==1) %>% select(Var2, Freq)
  prob <- table_prob$Freq
  names(prob) <- table_prob$Var2
  if(nrow(table_prob)>1){
  barplot(prob,legend.text = rownames(prob), main=paste(c("CANCER"), if_else(clus$sex_female[1] == "Men", "MALE","FEMALE" )),
          col=c("purple"))}
})
```
...
```{r, fig.width=10, fig.height=4}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- prop.table(table(clus$htn,clus$clusters))
  barplot(table_prob, beside=TRUE, legend.text = rownames(table_prob), main=paste("HYPERTENSION", if_else(clus$sex_female[1] == "Men","MALE", "FEMALE")),
          col=c("green","red"))
})
...
```{r, fig.width=10, fig.height=4}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- prop.table(table(clus$htn,clus$clusters), margin=2)
  table_prob <- as.data.frame(table_prob)
  table_prob <- table_prob %>% filter (Var1==1) %>% select(Var2, Freq)
  prob <- table_prob$Freq
  names(prob) <- table_prob$Var2
  barplot(prob,legend.text = rownames(prob), main=paste(c("HYPERTENSION"), if_else(clus$sex_female[1] == "Men", "MALE","FEMALE" )),
          col=c("purple"))
})
```
...
```{r, fig.width=10, fig.height=4}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- prop.table(table(clus$hf,clus$clusters))
  barplot(table_prob, beside=TRUE, legend.text = rownames(table_prob), main=paste("HEART FAILURE", if_else(clus$sex_female[1] == "Men","MALE", "FEMALE")),
          col=c("green","red"))
})
...
```{r, fig.width=10, fig.height=4}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- prop.table(table(clus$hf,clus$clusters), margin=2)
  table_prob <- as.data.frame(table_prob)
  table_prob <- table_prob %>% filter (Var1==1) %>% select(Var2, Freq)
  prob <- table_prob$Freq
  names(prob) <- table_prob$Var2
  barplot(prob,legend.text = rownames(prob), main=paste(c("HEART FAILURE"), if_else(clus$sex_female[1] == "Men", "MALE","FEMALE" )),
          col=c("purple"))
})
```
...
```{r, fig.width=10, fig.height=4}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- prop.table(table(clus$liver,clus$clusters))
  barplot(table_prob, beside=TRUE, legend.text = rownames(table_prob), main=paste("LIVER", if_else(clus$sex_female[1] == "Men","MALE", "FEMALE")),
          col=c("green","red"))
})
...
```{r, fig.width=10, fig.height=4}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- prop.table(table(clus$liver,clus$clusters), margin=2)
  table_prob <- as.data.frame(table_prob)
  table_prob <- table_prob %>% filter (Var1==1) %>% select(Var2, Freq)
  prob <- table_prob$Freq
  names(prob) <- table_prob$Var2
  barplot(prob,legend.text = rownames(prob), main=paste(c("LIVER"), if_else(clus$sex_female[1] == "Men", "MALE","FEMALE" )),
          col=c("purple"))
})
```
...
```{r, fig.width=10, fig.height=4}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- prop.table(table(clus$ra,clus$clusters))
  barplot(table_prob, beside=TRUE, legend.text = rownames(table_prob), main=paste("ARTHRITIS", if_else(clus$sex_female[1] == "Men","MALE", "FEMALE")),
          col=c("green","red"))
})
...
```{r, fig.width=10, fig.height=4}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- prop.table(table(clus$ra,clus$clusters), margin=2)
  table_prob <- as.data.frame(table_prob)
  table_prob <- table_prob %>% filter (Var1==1) %>% select(Var2, Freq)
  prob <- table_prob$Freq
  names(prob) <- table_prob$Var2
  barplot(prob,legend.text = rownames(prob), main=paste(c("ARTHRITIS"), if_else(clus$sex_female[1] == "Men", "MALE","FEMALE" )),
          col=c("purple"))
})
```
...
```{r, fig.width=10, fig.height=4}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- prop.table(table(clus$sleep_apnea,clus$clusters))
  barplot(table_prob, beside=TRUE, legend.text = rownames(table_prob), main=paste("SLEEP APNEA", if_else(clus$sex_female[1] == "Men","MALE", "FEMALE")),
          col=c("green","red"))
})
...
```{r, fig.width=10, fig.height=4}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- prop.table(table(clus$sleep_apnea,clus$clusters), margin=2)
  table_prob <- as.data.frame(table_prob)
  table_prob <- table_prob %>% filter (Var1==1) %>% select(Var2, Freq)
  prob <- table_prob$Freq
  names(prob) <- table_prob$Var2
  barplot(prob,legend.text = rownames(prob), main=paste(c("SLEEP APNEA"), if_else(clus$sex_female[1] == "Men", "MALE","FEMALE" )),
          col=c("purple"))
})
```
...
```{r, fig.width=10, fig.height=4}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- prop.table(table(clus$pcos,clus$clusters))
  if(clus$sex_female[1]=="Women"){
  barplot(table_prob, beside=TRUE, legend.text = rownames(table_prob), main=paste("POLYCYSTIC OVARY SYNDROME", if_else(clus$sex_female[1] == "Men","MALE", "FEMALE")),
          col=c("green","red"))}
})
...
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  ...
  names(prob) <- table_prob$Var2
  if(clus$sex_female[1]=="Women"){
  barplot(prob,legend.text = rownames(prob), main=paste(c("POLYCYSTIC OVARY SYNDROME"), if_else(clus$sex_female[1] == "Men", "MALE","FEMALE" )),
          col=c("purple"))}
})
```
...
```{r}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- table(clus$i.ep_AMI,clus$clusters)
  print(table_prob)
fisher.test(table_prob, simulate.p.value = TRUE)
})
```
...
```{r}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- table(clus$i.ep_TIA,clus$clusters)
  print(table_prob)
fisher.test(table_prob, simulate.p.value = TRUE)
})
```
...
```{r}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- table(clus$i.ep_StrokeI,clus$clusters)
  print(table_prob)
fisher.test(table_prob, simulate.p.value = TRUE)
})
```
```{r}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- table(clus$i.ep_Angor,clus$clusters)
  print(table_prob)
fisher.test(table_prob, simulate.p.value = TRUE)
})
```
...
```{r}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- table(clus$CKD,clus$clusters)
  print(table_prob)
fisher.test(table_prob, simulate.p.value = TRUE)
})
```
...
```{r}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- table(clus$liver,clus$clusters)
  print(table_prob)
fisher.test(table_prob, simulate.p.value = TRUE)
})
```
...
```{r}
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(clus){
  table_prob <- table(clus$ra,clus$clusters)
  print(table_prob)
fisher.test(table_prob, simulate.p.value = TRUE)
})
```
...
```{r}
library(devtools)
bbdd_clusters_8_sup %>% group_by(sex_female) %>% group_split() %>% map(function(dat){
...
## DISTRIBUCIÓN WEIGHT EN CLUSTERS
