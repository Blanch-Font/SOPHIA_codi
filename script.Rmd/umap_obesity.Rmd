...
bbdd %>%
  select(rowId, age, BMI, HbA1c, Leukocytes, cLDL, Tg, cHDL, DBP, SBP, Glucose, obesity_BMI) %>%
  group_by(obesity_BMI, .add = T) %>%
  group_split() %>% 
  map(~{.x %>%
      select(-c(rowId, obesity_BMI)) %>%
      cor %>%
      reshape2::melt() %>%
...
umap_res_15 <- bbdd %>%
  select(obesity_BMI, age, BMI, HbA1c, Leukocytes, cLDL, Tg, cHDL, DBP, SBP, Glucose) %>%
  group_by(obesity_BMI, .add = T) %>%
  group_split() %>% 
  map(function(strat, ...){
    umap(X = strat,
...
BIC_curve <- bbdd %>%
  ...
  map_dfr(function(strat){
    res <- Optimal_Clusters_KMeans(data = strat %>% select(-obesity_BMI), max_clusters = 1:20,
...
ggplot(km_res %>%
         pivot_longer(-c('obesity_BMI', 'X1', 'X2', 'km')) %>%
         group_by(name) %>%
  ...
  facet_wrap(~ obesity_BMI + name, scales = "free", nrow = 2) +
  theme_bw()
...
umap_res_15_resid <- bbdd_resid %>%
  select(obesity_BMI, HbA1c, Leukocytes, cLDL, Tg, cHDL, DBP, SBP, Glucose) %>%
  group_by(obesity_BMI, .add = T) %>%
  group_split() %>% 
  map(function(strat, ...){
    umap(X = strat,
