...
ggplot(bbdd_covar %>%
         select(rowId, age, BMI, HbA1c, Leukocytes, Monocytes, cLDL, Tg, cHDL, DBP, SBP, Glucose,
                Ferritin, sex_female, obesity_BMI) %>%
...
ind_cc <- complete.cases(bbdd_covar %>%
                           select(rowId, age, HbA1c, BMI, Ferritin,
...
bbdd <- bbdd_covar[ind_cc,] %>%
  select(rowId, age, BMI, HbA1c, Leukocytes, Monocytes, cLDL, cHDL, Tg, DBP, SBP, Glucose,
         Ferritin, sex_female, obesity_BMI, Current, Former,
...
bbdd %>%
  select(age, BMI, HbA1c, Leukocytes, Monocytes, cLDL, Tg, cHDL, DBP, SBP, Glucose, Ferritin,
         obesity_BMI) %>%
  group_by(obesity_BMI) %>% 
  group_modify(~{.x %>%
      cor %>%
...
umap_resid_50 <- bbdd_resid %>%
  select(HbA1c, Leukocytes, Monocytes, cLDL, Tg, cHDL, DBP, SBP, Glucose, Ferritin, obesity_BMI) %>%
  group_by(obesity_BMI, .add = T) %>%
  group_split() %>% 
  map(function(strat, ...){
    umap(X = strat,
...
gmm_res %>%
  map2(bbdd %>%
         select(HbA1c, Leukocytes, Monocytes, cLDL, Tg, cHDL, DBP, SBP, Glucose, Ferritin,
                obesity_BMI) %>%
         group_by(obesity_BMI, .add = T) %>%
         group_split(),
