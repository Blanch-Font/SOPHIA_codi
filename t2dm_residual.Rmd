...
```{r}
# setwd("C:/Users/20621066W/Desktop/T2DM/T2DM_as_residuals/")
```
...
umap_res %>%
    ...
    select(sex_female, rowId, X1, X2, age, BMI, TimeT2DM) %>%
    pivot_longer(c(age, BMI, TimeT2DM)) %>%
    split(f = .$name) %>%
    imap(
        ~{
            mp <- median(.x$value)
                ...
                map(
                    function(clusdat){
                        cov.wt(select(clusdat, HbA1c, Glucose, Leukocytes, Monocytes, cLDL,
                                      cHDL, Tg, DBP, SBP, Ferritin), wt = clusdat$value, cor = TRUE)
                    }
                )
        }
    )
...
diseases_surv <- clusters %>%
  map(function(data) {mod = coxph(Surv(t.ep_StrokeI, i.ep_StrokeI)~age + BMI+ TimeT2DM+ TimeC10+TimeHTNRx+cluster_maxprob , data=data)
...
diseases_surv <- clusters %>%
  map(function(data) {mod = coxph(Surv(t.ep_AMI, i.ep_AMI)~age +TimeT2DM+ BMI+TimeC10+TimeHTNRx+cluster_maxprob, data=data)
      tidy(mod, conf.int= TRUE, exponentiate=TRUE)})
head(diseases_surv)
```
...
diseases_surv <- clusters %>%
  map(function(data) {mod = coxph(Surv(t.ep_TIA, i.ep_TIA)~age + BMI+TimeT2DM+ TimeC10+TimeHTNRx+cluster_maxprob, data=data)
      tidy(mod, conf.int= TRUE, exponentiate=TRUE)})
head(diseases_surv)
```
...
diseases_surv <- clusters %>%
  map(function(data) {mod = coxph(Surv(t.ep_Angor, i.ep_Angor)~age + BMI+TimeT2DM+
                                    TimeC10+TimeHTNRx+ cluster_maxprob, data=data)
      tidy(mod, conf.int= TRUE, exponentiate=TRUE)})
head(diseases_surv)
```
...
```{r}
ggplot(bbdd_covar,aes(Ferritin, BMI)) + geom_point() +
  geom_smooth(method='lm', formula= y~x)
```
...
diseases_surv <- clusters %>%
  map(function(data) {mod = coxph(Surv(t.ep_AMI, i.ep_AMI)~age + BMI+TimeT2DM+
                                    TimeC10+TimeHTNRx+cluster_maxprob, data=data)
      tidy(mod, conf.int= TRUE, exponentiate=TRUE)})
head(diseases_surv)
```
