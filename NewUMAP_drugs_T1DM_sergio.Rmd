...
bbdd_covar <- bbdd_covar %>%
  mutate(obesity_BMI = as.numeric(obesity == 1 | 30 <= BMI),
         ...
         Time = TimeT1DM/365.25) %>% 
  mutate_at(.vars = c('A10', 'A10A', 'A10B', 'C01', 'C02', 'C03', 'C07', 'C08', 'C09', 'C10', 'M01A'),
            function(x) if_else(is.na(x), 0, x))

for (x in 1:nrow(bbdd_covar)){
  bbdd_covar$Time[x] <- max(bbdd_covar$TimeT1DM[x], bbdd_covar$TimeT2DM[x], na.rm = T)
}
bbdd_covar$Time[which(is.infinite(bbdd_covar$Time)==T)] <- NA
```
...
ind_cc <- complete.cases(bbdd_covar %>%
                           select(rowId, age, HbA1c, BMI, Time, Ferritin,
...
bbdd <- bbdd_covar[ind_cc,] %>%
  select(rowId, age, BMI, HbA1c, Leukocytes, Monocytes, cLDL, cHDL, Tg, DBP, SBP, Glucose,
         Time, sex_female, obesity_BMI, Current, Former, Ferritin, 
...
bbdd %>%
  select(rowId, age, BMI, HbA1c, Leukocytes, Monocytes, cLDL, Tg, cHDL, DBP, SBP, Glucose, Time,
         Ferritin,sex_female) %>%
  split(f = .$sex_female) %>% 
  map_dfr(~{.x %>% 
                ...
                map(
                    function(clusdat){
                        cov.wt(select(clusdat, HbA1c, Glucose, Leukocytes, Monocytes, cLDL,
                                      cHDL, Tg, DBP, SBP, Time, Ferritin), wt = clusdat$value, cor = TRUE)
                    }
                )
        }
    )
...
diseases <- bbdd_covar %>% select(eid, age, BMI,BMI, HbA1c, Glucose, Leukocytes, Monocytes,
                                         cLDL, cHDL, Tg, DBP, SBP, Time, Ferritin,  t.ep_StrokeI, i.ep_StrokeI,
...
dat <- arch %>% select(eid, cluster_maxprob) %>% left_join(dat, by="eid")
})
...
diseases_probs_vec <- bbdd_covar %>% select(eid, age, BMI,BMI, HbA1c, Glucose, Leukocytes, Monocytes,
                                         cLDL, cHDL, Tg, DBP, SBP, Time, Ferritin,  t.ep_StrokeI, i.ep_StrokeI, t.ep_AMI, i.ep_AMI, t.ep_Angor, i.ep_Angor, t.ep_TIA, i.ep_TIA, sex_female, ami, stroke, angor, tia, TimeC10,TimeHTNRx)
