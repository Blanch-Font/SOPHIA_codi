...
get_numeric_grp_diff <- function(variable, group) {
  # print(variable)
    tmp <- na.omit(df[, c(variable, group)])
    mod <- summary(aov(tmp[, variable] ~ tmp[, group]))

    out <- data.frame(
        variable = variable,
        group = group,
        F.value = round(mod[[1]][[4]][[1]], 3),
        df = round(mod[[1]][[1]][[1]], 3),
        p.value = ifelse(mod[[1]][[5]][[1]] < 0.001, "< 0.001", round(mod[[1]][[5]][[1]], 3)))
    return(out)
}

get_factor_grp_diff <- function(variable, group) {
    tmp <- na.omit(df[, c(variable, group)])
    mod <- chisq.test(table(tmp[, variable], tmp[, group]))
    out <- data.frame(
        variable = variable,
        group = group,
        chi.sq = round(mod[[1]][[1]], 3),
        df = round(mod[[2]][[1]], 3),
        p.value = ifelse(mod[[3]][[1]] < 0.001, "< 0.001", round(mod[[3]][[1]], 3)))
    return(out)
}
...
# ------------------------------------------------------------------------------
cat("Running statistical tests for numeric variables...\n")
# ------------------------------------------------------------------------------
...
for (i in group_vars) {
  ...
  if (i == 'card.angina'){
    names_colnames <- names_colnames[!names_colnames %in% c("hba1c", "fglucose", "bmi", "chol",
                                                            "hdl", "ldl", "tg", "creatinine",
                                                            "albumin", "EGFR", "ast", "alt",
                                                            "gamma.gt", "vit.d", "time.hba1c")]
  }
  if (i %in% c('neuro', 'dka')){
    names_colnames <- names_colnames[names_colnames != "albumin"]
  }
  numeric_grp_test <- do.call(rbind.data.frame,
                              mapply(get_numeric_grp_diff,
                                     names_colnames,
                                     group = i,
                                     SIMPLIFY = FALSE))

  rownames(numeric_grp_test) <- NULL

  openxlsx::write.xlsx(numeric_grp_test,
                       paste0(path, "numeric_grp_test_by_", i, ".xlsx"))
  # print("--------------------------------------------------------------------")
}

# ------------------------------------------------------------------------------
cat("Running statistical tests for factor variables...\n")
# ------------------------------------------------------------------------------

for (i in group_vars) {
  factor_grp_test <- do.call(rbind.data.frame,
                              mapply(get_factor_grp_diff,
                                     colnames(df[sapply(df, is.factor)]),
                                     group = i,
                                     SIMPLIFY = FALSE))
  ...
  openxlsx::write.xlsx(factor_grp_test,
                       paste0(path, "factor_grp_test_by_", i, ".xlsx"))
}
